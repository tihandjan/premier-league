class TeamsController < ApplicationController
    ...
    def show
        ...
        @news = Article.order('created_at DESC').where(team: @team.name, category: 'news').paginate(page: params[:page], per_page: 6)
        @articles = Article.order('created_at DESC').where(team: @team.name, category: 'articles').paginate(page: params[:page], per_page: 6)
        @videos = Video.order('created_at DESC').where('team_first = ? or team_second = ?', @team.name, @team.name).paginate(page: params[:page], per_page: 6)
